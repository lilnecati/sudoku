...
// Bu script, içeriği satır satır manipüle ederek ikinci difficultyValue3 değişkenini değiştirir
...
do {
    ...
    // 827. satırdaki tanımı değiştir (tam satır numarası değişmiş olabilir, bu yüzden search kullanıyoruz)
    var found = 0
    
    for i in 0..<lines.count {
        if lines[i].contains("let difficultyValue3: SudokuBoard.Difficulty") {
            found += 1
            // İkinci bulduğumuz tanımı değiştirelim
            if found == 2 {
                lines[i] = lines[i].replacingOccurrences(of: "difficultyValue3", with: "difficultyValue4")
                
                // Sonraki 10 satırda difficultyValue3 referanslarını da değiştirelim
                var j = i + 1
